"""Experiment: lower ``async def``/``await`` to plain generators.

This deliberately has no dependency on asyncio.  It tests whether coroutine
syntax can be used as a source-level notation for a cooperative, synchronous
effect protocol: lowered functions are ordinary generator functions, ``await``
becomes ``yield from``, and :class:`Scheduler` drives callback effects.

It is an experiment, not a proposed public API.  In particular, this only
suspends frames that have been lowered.  An unmodified synchronous function
which calls a suspension point still cannot have its Python stack captured.
"""
...
class LoweringError(SyntaxError):
    """Raised when source uses async semantics this experiment cannot lower."""
...
def lower_ast(source: str, filename: str = "<ast-lowering-experiment>") -> ast.Module:
...
def compile_lowered(source: str, filename: str = "<ast-lowering-experiment>") -> CodeType:
...
def exec_lowered(
    ...
    filename: str = "<ast-lowering-experiment>",
) -> dict[str, Any]:
    ...
    exec(compile_lowered(source, filename), namespace)  # noqa: S102 - this is the experiment
    return namespace
