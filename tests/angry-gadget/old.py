...
def main():
    ...
    analysis_cache = Path("analysis.pkl")
    p = angr.Project(args.binary, auto_load_libs=False)

    print("Building CFG, this will take a moment")
    p.analyses.CFG(data_references=True, cross_references=True, resolve_indirect_jumps=False)
...
def check_execve(state):
    ...
    rsi_val = state.memory.load(state.regs.rsi, 8)
    constraints = [
...
def pretty_print(gadgets):
    for addr, constraints in gadgets:
        print("{} :".format(
            "libc_base + " + str(hex(addr)), 'cyan', attrs=['bold']))
        for constraint in constraints:
            print("\t{}".format(constraint, 'red', attrs=['bold']))
