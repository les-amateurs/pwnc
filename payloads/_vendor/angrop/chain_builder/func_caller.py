...
class FuncCaller(Builder):
    ...
    def _canonical_reg_name(self, reg_name):
        """Translate calling-convention aliases to angrop's register names."""
        try:
            offset = self.project.arch.get_register_offset(reg_name)
            canonical = self.project.arch.translate_register_name(offset)
        except (KeyError, ValueError):
            return reg_name
        if canonical in self.chain_builder.arch.reg_list:
            return canonical
        return reg_name
    ...
    def _func_call(self, func_gadget, cc, args, extra_regs=None, preserve_regs=None,
        ...
        if needs_return and isinstance(cc.RETURN_ADDR, SimRegArg) and cc.RETURN_ADDR.reg_name != 'ip_at_syscall':
            reg_name = self._canonical_reg_name(cc.RETURN_ADDR.reg_name)
        ...
        # A terminal stack-argument call still needs the callee's entry frame:
        # the return-address slot followed by its arguments.  Upstream used to
        # return before emitting either, which silently turned i386
        # ``exit(status)`` into ``exit(stack_garbage)``.
        if not needs_return:
            if stack_arguments:
                if not isinstance(cc.RETURN_ADDR, SimStackArg):
                    raise RopException("stack arguments with a non-stack return address are unsupported")
                for _ in range(func_gadget.stack_change//arch_bytes):
