...
_INITIAL_STATE_PLUGIN_LOCK = threading.RLock()
...
def make_initial_state(project, stack_gsize):
    ...
    # SimState's default-plugin registry is process-global.  Preserve the
    # caller's exact plugin and serialize the brief construction window so an
    # exception or concurrent analysis cannot leak SpecialMem into host code.
    with _INITIAL_STATE_PLUGIN_LOCK:
        preset = angr.SimState._presets["default"]
        previous_sym_memory = preset._default_plugins["sym_memory"]
        ...
        try:
            ...
            angr.SimState.register_default("sym_memory", previous_sym_memory)
