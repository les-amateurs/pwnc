...
def worker_func1_batch(cslices):
    count = 0
    results = []
    for cslice in cslices:
        n, batch = worker_func1(cslice)
        count += n
...
class GadgetFinder:
    ...
    def _multiprocess_static_analysis(self, processes, show_progress, timeout):
        ...
        batches = []
        batch = []
        for cslice in self._truncated_slices():
            batch.append(cslice)
            if len(batch) == 40:
            ...
            try:
                ...
                for addr, h in results:
                    ...
                    if h:
                        ...
                        todos.append(addr)
            except BaseException as exc:  # preserve callback failures for the waiting thread
                with progress:
                    errors.append(exc)
                    progress.notify_all()
                return
        ...
        with mp.Pool(processes=processes, initializer=_set_global_gadget_analyzer, initargs=initargs) as pool:
            for slices in batches:
                pool.apply_async(worker_func1_batch, args=(slices,), callback=on_success, error_callback=on_error)
            pool.close()

            with progress:
                remaining = None if timeout is None else max(0, start + timeout - time.monotonic())
                progress.wait_for(lambda: completed[0] == len(batches) or bool(errors), timeout=remaining)
    ...
    def _analyze_gadgets_multiprocess(self, processes, tasks, show_progress, timeout, cond_br):
        ...
        progress = threading.Condition()
        sync_data = [time.monotonic(), 0]
        def on_success(gs):
            with progress:
                gadgets.extend(gs)
                if t:
                    t.update(1)
                sync_data[0] = time.monotonic()
                sync_data[1] += 1
                progress.notify_all()
        ...
        with mp.Pool(processes=processes, initializer=_set_global_gadget_analyzer, initargs=initargs) as pool:
            ...
            with progress:
                while sync_data[1] != len(tasks):
                    now = time.monotonic()
                    stall_timeout = (
                        ANALYZE_GADGET_TIMEOUT
                        if sync_data[1] > len(tasks)*0.8
                        else ANALYZE_GADGET_TIMEOUT*5
                    )
                    remaining = sync_data[0] + stall_timeout - now
                    if timeout is not None:
                        remaining = min(remaining, start + timeout - now)
                    if remaining <= 0:
                        break
                    progress.wait(timeout=remaining)
