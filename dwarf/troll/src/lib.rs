...
type Handle = Arc<Data>;

#[unsafe(no_mangle)]
extern "C" fn test(n: i32) -> *mut Handle {
    return Box::into_raw(Box::new(Arc::new(Data { inner: n })));
}

#[unsafe(no_mangle)]
extern "C" fn acquire(h: &mut Handle) -> bool {
    println!("h = {:#x}", h as *const _ as usize);
    println!("h = {:?}", Arc::strong_count(h));
    println!("h = {:?}", Arc::weak_count(h));

    if let Some(n) = Arc::get_mut(h) {
        unsafe { Arc::increment_strong_count(Arc::as_ptr(h)) }
        true
    } else {
        false
    }
